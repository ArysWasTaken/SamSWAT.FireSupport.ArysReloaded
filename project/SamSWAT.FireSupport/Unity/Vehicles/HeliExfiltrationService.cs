...
public sealed class HeliExfiltrationService(FireSupportSpotter spotter, int maxRequests) : FireSupportService(maxRequests)
{
	...
	private async UniTaskVoid ConfirmRequest(Vector3 position, CancellationToken cancellationToken)
	{
		...
		await UniTask.WaitForSeconds(35f + PluginSettings.HelicopterWaitTime.Value, cancellationToken: cancellationToken);
